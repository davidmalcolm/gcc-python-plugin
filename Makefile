...
testdemo: plugin
	...
	gccversion=$(shell ./print-gcc-version); \
	echo "Demo: gcc version: $$gccversion"; \
	if [ $$gccversion -ge 7000 ]; then \
		ref=demo.expected.no-refcounts; \
	else \
		ref=demo.expected; \
	fi ; \
	echo "Demo: reference file used: $$ref"; \
	diff $$ref demo.filtered
