...
LIBGCCCAPI_SO	:= gcc-c-api/libgcccapi.so

# In a full install, libgcccapi.so would be installed to the GCC plugin
# directory, but when invoking the plugin from a working copy we need
# this so it can find it:
...
$(GCC_PYTHON_PLUGIN_SO): $(PLUGIN_OBJECT_FILES) $(LIBGCCCAPI_SO)
	$(CC) $(CPPFLAGS) $(CFLAGS) $(LDFLAGS) -shared $(PLUGIN_OBJECT_FILES) -o $@ $(LIBS) -lgcccapi -Lgcc-c-api

$(LIBGCCCAPI_SO):
	cd gcc-c-api && make libgcccapi.so
...
$(PLUGIN_OBJECT_SOURCE_FILES) $(PLUGIN_OBJECT_GENERATED_FILES): %.o: $(srcdir)%.c autogenerated-config.h $(srcdir)gcc-python.h $(LIBGCCCAPI_SO) autogenerated-EXTRA_CFLAGS.txt
