...
PROPOSED_API_SOURCE_XML:= \
  ...
  proposed-plugin-api/function.xml \
  proposed-plugin-api/gimple.xml \
  proposed-plugin-api/location.xml \
  proposed-plugin-api/rtl.xml \
  proposed-plugin-api/tree.xml \
  proposed-plugin-api/type.xml \
  proposed-plugin-api/variable.xml
...
PROPOSED_API_GENERATED_HEADERS:= \
  ...
  proposed-plugin-api/gcc-function.h \
  proposed-plugin-api/gcc-gimple.h \
  proposed-plugin-api/gcc-location.h \
  proposed-plugin-api/gcc-rtl.h \
  proposed-plugin-api/gcc-tree.h \
  proposed-plugin-api/gcc-type.h \
  proposed-plugin-api/gcc-variable.h
...
$(PLUGIN_OBJECT_SOURCE_FILES): %.o: $(srcdir)%.c autogenerated-config.h $(srcdir)gcc-python.h $(PROPOSED_API_GENERATED_HEADERS)
	$(COMPILE.c) $(OUTPUT_OPTION) $<

clean:
	$(RM) *.so *.o autogenerated* $(PROPOSED_API_GENERATED_HEADERS)
	$(RM) -r docs/_build
...
$(PROPOSED_API_GENERATED_HEADERS): $(PROPOSED_API_SOURCE_XML) proposed-plugin-api/xml-to-h.py proposed-plugin-api/xmltypes.py
	cd $(srcdir)proposed-plugin-api && $(PYTHON) xml-to-h.py
...
check-api:
	xmllint --noout --relaxng $(srcdir)./proposed-plugin-api/api.rng $(srcdir)./proposed-plugin-api/*.xml
