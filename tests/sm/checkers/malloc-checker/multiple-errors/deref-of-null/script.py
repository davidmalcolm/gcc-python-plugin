...
def selftest(ctxt, solution):
    ...
    node_write_to_p = ctxt.get_nodes().assigning_constant(0).one()
    ssaname = node_write_to_p.stmt.lhs

    p = ctxt.find_var(node_write_to_p, 'p')
    node_write_to_p_i = ctxt.get_nodes().assigning_constant(1).one()
    ctxt.assert_statenames_for_expr(node_write_to_p_i, p,
                                    frozenset(['ptr.null']))

    # It should now have given up, with the p in the "ptr.stop" state:
